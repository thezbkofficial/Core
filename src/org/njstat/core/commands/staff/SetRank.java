			...
			if (cmd.getName().equalsIgnoreCase("setrank")) {
				...
				if ((sender instanceof Player) || (sender instanceof ConsoleCommandSender)) {
					if (args.length == 0) {
						...
						if (!(fileManager.getrankConfig()
								.getBoolean("groups.owner.users." + p.getUniqueId().toString()))) {
							sender.sendMessage(
									ChatColor.translateAlternateColorCodes('&', prefix.RANKS + message.NOPERMISSION));
							...
							if (args.length > 0) {
								...
								if (target == null) {
									p.sendMessage(prefix.CORE + message.NOTONLINE);
									return false;
